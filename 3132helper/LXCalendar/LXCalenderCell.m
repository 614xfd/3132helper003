...
-(void)setModel:(LXCalendarDayModel *)model{
    ...
    for (UIView *v in self.proportionView.subviews) {
...
- (void)refreshView{
    ...
    NSArray *dataList = [self stringToJSON:self.scheduleObj.affairJson];
    CGFloat w = self.frame.size.width - 5 * 2;
    for (int index = 0; index < (dataList.count >= 3?3:dataList.count); index++) {
        NSDictionary *dict = dataList[index];
        UIView *bView = [[UIView alloc]initWithFrame:CGRectMake(5, 1.5 + (8 + 3) * index, w, 8)];
        ...
        [self.proportionView addSubview:bView];
        
        
        UIView *scheduleView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, bView.frame.size.width * [dict[@"completeness"] floatValue], bView.frame.size.height)];
