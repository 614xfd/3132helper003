...
@property(nonatomic,strong)ScheduleObject *cacheModel;
@end
...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        self.cacheModel = [[ScheduleObject alloc]init];
        //通过模型生产操作者
        
        if (!self.sqOperator) {
            self.sqOperator = [TDOperator instanceWhithModel:self.cacheModel];
        }
    }
    return self;
}
...
-(void)layoutSubviews{
    ...
    self.calendarHeader.frame = CGRectMake(0, 0, self.lx_width, 70);
}
#pragma mark---懒加载
-(LXCalendarHearder *)calendarHeader{
    if (!_calendarHeader) {
        _calendarHeader =[LXCalendarHearder showView];
        _calendarHeader.frame = CGRectMake(0, 0, self.lx_width, 70);
...
-(UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        flow.itemSize = CGSizeMake(self.lx_width/7, 70);
        _collectionView =[[UICollectionView alloc]initWithFrame:CGRectMake(0, self.calendarWeekView.lx_bottom, self.lx_width, 6 * 70) collectionViewLayout:flow];
